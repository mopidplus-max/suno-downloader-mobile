      ...
      if (track.coverUrl != null) {
        try {
          track = track.copyWith(
            coverBytes: await _download.bytes(track.coverUrl!),
          );
        } catch (_) {}
      }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final edited = TrackMetadata(
        ...
        fileName:
            '${SunoService.sanitizeFileName(fileName.replaceAll(RegExp(r'\.mp3$', caseSensitive: false), ''))}.mp3',
      );
          ...
          SliverPadding(
            ...
            sliver: SliverList.list(
              children: [
                ...
                const Text(
                  'Вставьте ссылку, проверьте метаданные и сохраните MP3 в выбранную папку.',
                  style: TextStyle(
                    fontSize: 16,
                    height: 1.5,
                ...
                const SizedBox(height: 12),
                FilledButton.icon(
                  onPressed: _state == ScreenState.idle ? _load : null,
                  icon: const Icon(Icons.arrow_downward_rounded),
                  label: const Text('Получить песню'),
  ...
  Widget build(BuildContext context) => Row(
    children: [
      Container(
        ...
        child: const Icon(Icons.graphic_eq_rounded, color: AppTheme.lime),
      ),
      ...
      const Expanded(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Только ваши и разрешённые треки',
...
class _EditorCard extends StatelessWidget {
  ...
  final TextEditingController title, artist, album, year, fileName;
  final VoidCallback onCover, onSave;
  @override
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      children: [
        ...
        TextButton.icon(
          ...
          label: const Text('Заменить обложку PNG / JPEG'),
        ),
        const SizedBox(height: 8),
        _field(fileName, 'Название файла', Icons.audio_file_rounded),
        _field(title, 'Название', Icons.music_note_rounded),
        _field(artist, 'Автор', Icons.person_rounded),
        _field(album, 'Альбом', Icons.album_rounded),
        _field(
          year,
          'Год',
...
class _DoneCard extends StatelessWidget {
  ...
  final String folder;
  final VoidCallback onShare, onAgain;
  @override
  Widget build(BuildContext context) => Container(
    padding: const EdgeInsets.all(24),
    decoration: BoxDecoration(
      ...
      children: [
        const Align(
          ...
          child: CircleAvatar(
            radius: 30,
            backgroundColor: AppTheme.lime,
            child: Icon(Icons.check_rounded, color: AppTheme.ink, size: 34),
          ),
        ),
        ...
        Text(
          'Сохранено в папку\n$folder',
          style: const TextStyle(color: Color(0xFFC9CEC9), height: 1.5),
        ),
